...
internal class SmallPersistentVector<E>(private val buffer: Array<Any?>) : ImmutableList<E>, AbstractImmutableList<E>() {
    ...
    override fun add(element: E): ImmutableList<E> {
        ...
        tail[0] = element
        return PersistentVector(Node(buffer, null), tail, size + 1, 0)
    }
    ...
    override fun addAll(index: Int, c: Collection<E>): ImmutableList<E> {
        ...
        if (size + c.size <= MAX_BUFFER_SIZE) {
            ...
            System.arraycopy(buffer, index, newBuffer, index + c.size, size)
            var position = index
            for (element in c) {
    ...
    override fun add(index: Int, element: E): ImmutableList<E> {
        ...
        val rootBuffer = buffer.copyOf()
        System.arraycopy(buffer, index, rootBuffer, index + 1, size - index - 1)
        rootBuffer[index] = element
        val tail = arrayOfNulls<Any?>(MAX_BUFFER_SIZE)
        tail[0] = buffer[MAX_BUFFER_SIZE_MINUS_ONE]
        return PersistentVector(Node(rootBuffer, null), tail, size + 1, 0)
    }
