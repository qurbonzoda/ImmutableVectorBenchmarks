...
open class Add {
    ...
    private var emptyVector = PersistentVector.EMPTY

    @Benchmark
    fun addLast(): PersistentVector {
    ...
    @Benchmark
    fun addLastAndIterate(bh: Blackhole): PersistentVector {
        var vector = this.emptyVector
        repeat(times = this.listSize) {
            vector = vector.cons("some element")
        }
    ...
    @Benchmark
    fun addLastAndGet(bh: Blackhole): PersistentVector {
        var vector = this.emptyVector
        repeat(times = this.listSize) {
            vector = vector.cons("some element")
        }
